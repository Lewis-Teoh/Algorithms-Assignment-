...
class Graph:
    ...
    def find_transit(self):
        ...
        start_string_matching_time = time.time()
        print("Start String Matching Execution : ",start_string_matching_time)
        sentimenscore = []
        for i in range(len(self.route)):
        ...
        end_string_matching_time = time.time()
        print("End String Matching Execution : ", end_string_matching_time)
        print("String Matching Time Execution : ",round(end_string_matching_time-start_string_matching_time,4)," second")
        # print("Sentimenscore",sentimenscore)
        ...
        for i in range(len(self.path_travel)):
            ...
            self.path_travel[i].append(round(sentimenscore[i],2))
    ...
    def save_item(self):
        ...
        for i in range(len(self.path_travel)):
            self.an_item = dict(distance =self.path_travel[i][0] , route=self.path_travel[i][1] ,sentimentscore = self.path_travel[i][3], score=self.path_travel[i][2])
            items.append(self.an_item)

        #selection sort table by score
        for i in range(len(items)):
            minimum = i

            for j in range(i+1 , len(items)):
                if items[j]['score'] > items[minimum]['score'] :
                    minimum = j
...
def algo():
    if request.method == 'POST':
        ...
        print("Start Find Path Time: ",find_path_time)
        g = Graph(15)
        g.printAllPaths(0, int(destination))
        end_find_path_time = time.time()
        print("End Find Path Time: ", end_find_path_time)
        print("Find path time execution : ",round(end_find_path_time-find_path_time,4)," second")
